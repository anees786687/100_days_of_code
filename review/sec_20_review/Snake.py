...
class Snake():
    ...
    def check_collision_food(self):
        ...
        if math.sqrt((self.snake_head.xcor() - self.food_obj.xcor()) ** 2 + (self.snake_head.ycor() - self.food_obj.ycor()) ** 2) < 5.0:
            ...
            # simply call the spawn food method of the food class
            self.food_obj.ht()
            del self.food_obj
            self.food_obj = Food()

            # once food is eaten, extend the snake body by adding a turtle to the snake_body list
            ...
            if self.direction == 'east':
                extended_body.setx(self.snake_head.xcor() - self.body_size * self.sub_body_offset)
                self.snake_body.append(extended_body)
            elif self.direction == 'north':
                extended_body.setx(self.snake_head.ycor() - self.body_size * self.sub_body_offset)
                self.snake_body.append(extended_body)
            elif self.direction == 'west':
                extended_body.setx(self.snake_head.xcor() + self.body_size * self.sub_body_offset)
                self.snake_body.append(extended_body)
